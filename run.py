...
def display_main_menu_and_get_user_choice():
    ...
    while True:
        ...
        if choice == '1':
            """
            Allow user to chose to view full inventory or inventory
            of specific date.
            Run a while loop to request valid choice from the user
...
def view_full_inventory():
    """
    Display full inventory to user, using for loop and format()
    to make data displayed more user friendly.
    """
    ...
    for i in range(len(inventory_data)):
        if i == 0:
            ...
            print('{:<12s}{:>6s}{:>6s}{:>6s}{:>6s}{:>6s}{:>6s}'.format(
                inventory_data[i][0], inventory_data[i][1],
                inventory_data[i][2], inventory_data[i][3],
                inventory_data[i][4], inventory_data[i][5],
                inventory_data[i][6])
            )
            print(dash)
        else:
...
def view_specific_date_inventory():
    """
    Display inventory of item of specific date
    according to the expiry date user provided.
    For loop and format() are used to make data
    displayed more user friendly.
    """
    while True:
        print("Please enter expiry date of the milk")
        print("you'd like to view(dd-mm-yyyy)")
...
def add_item_from_delivery():
    """
    Collect delivery data from user to update delivery and
    inventory worksheet and run a funciton to get expiry date
    to update usage and wastage worksheet date list for future use.
    ...
    while True:
        ...
        print("Data should begin with expiry date in the")
        print("format of dd-mm-yyyy and quantity")
        print("to each location B1, Y1, R1, B2, Y2, R2")
        print("respectively separated by commas.")
        print("Please note: quantity for each location")
        print("cannot exceed 100 unit. ")
        ...
        if validate_delivery_data(
            delivery_data, date_value,
            date_value_as_list,
            delivery_quantity_values
                ):
            print("Data is valid!\n")
            break
...
def get_expiry_date_of_delivery_to_update_usage_wastage_sheet(data):
    """
    Get expiry date of new delivery to update usage
    and wastage worksheet for future use.
    """
    new_expiry_date = data[0]
    new_expiry_date_to_update_usage_wastage = [
        new_expiry_date, '0', '0', '0', '0', '0', '0'
        ]

    update_worksheet(
        new_expiry_date_to_update_usage_wastage, "remove_by_using"
            )
    update_worksheet(
        new_expiry_date_to_update_usage_wastage, "remove_by_wasting"
            )
...
def remove_item_by_usage(worksheet, process):
    """
    Allow user to record milk usage by calling a function to request date,
    location and quanity and in turn call for another function to validate
    the data provided which if pass then call a funciton to perform
    simple calculation on relavent worksheet.
    """
    get_milk_data_to_be_removed_and_called_remove_and_add_function(
        worksheet, process
            )
...
def remove_item_by_wastage(worksheet, process):
    """
    Allow user to record milk wastage by calling a function to request date,
    location and quanity and in turn call for another function to validate
    the data provided which if pass then call a funciton to perform
    simple calculation on relavent worksheet.
    """
...
def record_redistribution():
    """
    Allow user to enter data to record redistribution of milk between
    usage locations and update inventory worksheet accordingly.
    This allow user to move inventory around so oldest date are used
    up while keeping track of the movement.
    All data provideded are validate before a function
    for relavent calculation is called.
    """
    while True:
        print("Please enter expiry date of the milk")
        print("to be redistributed (dd-mm-yyyy)")
    ...
    print("Please enter the location (B1 or Y1 or R1 or B2 or Y2 or R2")
    print("where milk is taken from")
    ...
    number_bottle_value = get_quantity_of_milk_removed_or_added(
        date_input_value, location_value_from, 'inventory'
            )
    print("Please enter the location (B1 or Y1 or R1 or B2 or Y2 or R2)")
    print("where milk is taken to")
    location_value_to = get_location_of_milk_removed_or_added()

    remove_inventory_from_worksheet(
        date_input_value, location_value_from, number_bottle_value, "inventory"
            )
    add_inventory_to_worksheet(
        date_input_value, location_value_to, number_bottle_value, "inventory"
            )

    print(f"{number_bottle_value} bottle(s) of milk has been subtracted ")
    print(f"from {location_value_from} and added to {location_value_to} ")
    print("successfully...\n")
...
def get_milk_data_to_be_removed_and_called_remove_and_add_function(
    worksheet_to_add, process
        ):
    """
    Collect usage or wastage data from user to calculate usage or wastage
    in respective location, by using data collect to help locate cell
    coordinate to be updated. The data collected is also used to
    update inventory worksheet.
    """
    while True:
        print(f"Please enter expiry date of the milk to be {process}")
        print("(dd-mm-yyyy)")
    ...
    print("Please enter the location (B1 or Y1 or R1 or B2 or Y2 or R2)")
    print(f"for the milk to be {process}")
    location_value = get_location_of_milk_removed_or_added()
    print(f"Please enter the number of milk bottle to be {process}")
    number_bottle_value = get_quantity_of_milk_removed_or_added(
        date_input_value, location_value, 'inventory'
            )

    add_inventory_to_worksheet(
        date_input_value, location_value,
        number_bottle_value, worksheet_to_add
            )
    remove_inventory_from_worksheet(
        date_input_value, location_value,
        number_bottle_value, 'inventory'
            )
...
def add_inventory_to_worksheet(date, location, quantity, worksheet):
    """
    Use value input by user to locate cell row
    and column to perform addition to.
    """
...
def remove_inventory_from_worksheet(date, location, quantity, worksheet):
    """
    Use value input by user to locate cell row and
    column to perform subtraction from.
    """
...
def get_quantity_of_milk_removed_or_added(date, location, worksheet):
    ...
    while True:
        ...
        if validate_number_of_bottle_input(
            date, location, number_of_bottle, worksheet
                ):
...
def validate_choice_input(value):
    """
    Inside the try, raises ValueError if value input
    is an empty string or invalid choice.
    """
    try:
        if value == " ":
            raise ValueError(
...
def validate_delivery_data(values, date_value, date_value_as_list, quantity):
    """
    Validate if the data input follow the correct
    format and maximum quanity allow for milk is adhered to.
    Inside the try, raises ValueError if data is not equal
    to 7 strings and if the quanity to each location exceed 100.
    """
    try:
        if len(values) != 7:
            raise ValueError(
        ...
        for x in quantity:
            if int(x) > 100:
                raise ValueError(
                    f"Quantity entered for delivery cannot "
                    f"exceed 100 for each location"
                )
...
def validate_if_date_exist(worksheet, date):
    """
    Validate to check if date provided by the user exist, if not
    the date will not be accepted and request for date that already existed.
    The function also called for date provided to be checked to see
    if the date is valid and follow correct pattern or not.
    """
    ...
    try:
        ...
        if date not in worksheet.col_values(1):
            raise ValueError(f"Date does not exist")
...
def validate_date_input(date_value, date_value_as_list):
    """
    Inside the try, check if date input is not empty and that
    date is valid and follow correct pattern.
    ValueError is returned and used by the funciton that called
    this validation method to display error or pass the validation process.
    """
    if len(date_value) != 10:
        raise ValueError(
            f"Exactly 10 characters required in the format "
            f"of dd-mm-yyyy for date, you provided "
            f"{len(date_value)} characters"
            )
    ...
    if date_value.count("-") != 2:
        raise ValueError(
            f"The date value require two '-' one "
            f"after day and one after month, "
            f"you provided {date_value.count("-")}"
        )
...
def validate_location_input(value):
    """
    Inside the try, check if value entered is not in
    the dictionary that represent all locations in
...
def validate_number_of_bottle_input(date, location, quantity, worksheet):
    """
    Inside the try, check if the quantity entered is equal
    to 0 or empty string if it is then ValueError is raised.
    The quantity provided is also checked to make sure only
    quanity less than or equal to quanity available in inventory for
    respective location is entered if not an error message is displayed.
    """
    ...
    column = cell_location2.col
    cell_value_to_compare_with = int(
        worksheet_to_validate_quantity.cell(row, column).value
            )

    try:
        if int(quantity) > cell_value_to_compare_with:
            raise ValueError(
                f"Qunantity to remove cannot exceed "
                f"what is available in the inventory"
            )
        if quantity == " ":
            raise ValueError(
                f"Input value can not be empty"
